...
void testPrimMST() {
    ...
    MatrixUDG<char> graph(vertices, 3, matrix);
...
void testErrorHandling() {
    ...
    try {
        MatrixUDG<char> graph(vertices, 2, edges, 1);
        assert(false && "Should throw exception for invalid vertex");
    } catch (const std::invalid_argument&) {
        // Expected exception
    }

    MatrixUDG<char> graph(vertices, 2, edges, 0);
    try {
        graph.Prim(-1);
...
void testDisconnectedGraph() {
    ...
    MatrixUDG<char> graph(vertices, 4, edges, 2);
